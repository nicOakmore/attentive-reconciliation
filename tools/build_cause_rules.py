...
IN = ['ret_named_known', 'ret_missing_abs', 'ret_unnamed_abs', 'ret_zero', 'ti_gap', 'wrong_col_match',
      ...
      'census_found', 'census_ss', 'census_med', 'upstream_cause']
...
ROWS = [
    ...
    row("federal saving differs with nothing upstream to explain it",
        {"fed_in_doubt": N, "federal_gap_abs": gt(1.0), "upstream_cause": N},
        "The proposal's federal saving does not match payroll", "{federal_gap}", "fed_mismatch"),
    # ---- residual component differences -------------------------------------------------
    row("state withholding differs materially",
        {"state_gap_abs": gt(1.0), "upstream_cause": N},
        "State withholding does not match the proposal", "{state_gap}", "state_generic"),
    row("Social Security and Medicare differ with nothing upstream to explain it",
        {"fica_gap_abs": gt(1.0), "ss_mismatch": N, "med_mismatch": N, "upstream_cause": N},
        "Social Security and Medicare withheld do not match the proposal", "{fica_gap}", "fica"),
]
...
DETAILS = {
    ...
    "premium_not_pretax": "Of the {premium_m} premium, {premium_pretax_gap} a month stayed inside federal taxable wages. Payroll must take the whole premium pre-tax; no census change fixes this.",
    "premium_not_medicare": "Of the {premium_m} premium, {medg_gap} a month stayed inside Medicare wages. Payroll must take the whole premium out of Medicare wages; no census change fixes this.",
    ...
    "reimb_partial": "The reimbursement differs from the {premium_m} premium by {reimb_gap} a month and that difference hits take home pay. Payroll must set the reimbursement equal to the premium.",
    "ret_changed": "The retirement deduction moved {ret_change} a month between the two payslips, so the premium's own effect cannot be isolated. Ask payroll for a mock that changes only the premium.",
    ...
    "fed_zero": "This employee had {fed_before_m} a month of federal tax and none after the premium, so the federal saving cannot exceed {fed_before_m}. Build the promise on that amount.",
    ...
    "gross_moved": "Gross pay moved {gross_moved} a month between the two payslips, so more than the premium changed. Ask payroll for a mock that changes only the premium.",
    ...
    "fed_mismatch": "The report promises {eng_fed_sav} a month of federal saving; payroll saved {pay_fed_sav}. No census field explains the difference. Compare the W-4 details and additional federal withholding on the census against the payslip, correct the census and rerun the proposal.",
    ...
    "state_generic": "State withholding differs from the proposal by {state_gap} a month. Correct the census state marital status and withholding dependents to match the payslip and rerun the proposal.",
    "fica": "Social Security and Medicare withheld differ from the proposal by {fica_gap} a month. Check the census pay frequency against the payslip and rerun the proposal.",
}
